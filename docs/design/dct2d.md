## Fast Discrete Cosine Transforms (1D & 2D)

source: https://clima.github.io/OceananigansDocumentation/stable/numerical_implementation/elliptic_solvers/#Cosine-transforms-on-the-GPU
...
In this section we will be using the DCT-II as the definition of the forward cosine transform for a real signal of length $N$
```math
    \begin{equation}
    % \label{eq:FCT}
